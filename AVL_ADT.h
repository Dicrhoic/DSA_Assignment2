...
class AvlTree
	{
	 private:
	    ...
	    NODE<TYPE> *_compareString  (KTYPE        key,
	    							 NODE<TYPE>  *root);
...
//  	The following function is used for debugging.
	 ...
	 public:
	    ...
	    bool AVL_UpdateAndInsert(string word, vector<int>relevantIDs);
	    bool AVL_Update  (KTYPE   key, TYPE dataOut);
//      The following function is used for debugging.
	    void  AVL_Print    (void);
	} ; // class AvlTree
...
template <class TYPE, class KTYPE>
bool AvlTree<TYPE,KTYPE> ::AVL_UpdateAndInsert(string word, vector<int>relevantIDs){
	NODE<TYPE> *newPtr;
	NODE<TYPE> *node;
	bool taller;

	node = _compareString(word, tree);
	if (node){
		TYPE& id_data = node->data;

		for (int i = 0; i < relevantIDs.size(); i++){
			bool idExists = false;

			for (int j = 0; j < id_data.idList.size(); j++){ //goes through data's idList
				if (relevantIDs[i] == id_data.idList[j] ){
					idExists = true;
				}
			}

			if (!idExists){
				id_data.idList.push_back(relevantIDs[i]);
				node->data = id_data;
			}
		}
	}
...
/*===================== _compareString =====================
 * Built off the basis of _retrieve compares if the key is greater or
 * lesser than
 *
 * */
template<class TYPE, class KTYPE>
NODE<TYPE>*  AvlTree<TYPE, KTYPE>
        ::	_compareString (KTYPE       key,
                       NODE<TYPE> *root)
{
	if(root)
	{
		if (key.compare(root->data.key) < 0){
			return _compareString (key, root->left);
		}
		else if (key.compare(root->data.key) > 0){
			return _compareString (key, root->right);
		}
		else
			//Found equal key
			return (root);
...
template <class TYPE, class KTYPE>
bool   AvlTree<TYPE, KTYPE>::  AVL_Update  (KTYPE   key, TYPE dataOut)
{
    //	Local Definitions
    NODE<TYPE> *node;
